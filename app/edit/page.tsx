...
import { Palette, Move, Filter, Sun, Moon, Frame, Layers, Crop, X, Bookmark } from "lucide-react"
...
// Import types
import type { ImageEffects, BackgroundData } from "../types/image"
...
export default function EditPage() {
  ...
  const handleSliderChange = (property: keyof ImageEffects, value: number | boolean) => {
  ...
  const handleShadowChange = (property: keyof ImageEffects["shadow"], value: any) => {
    setImageEffects((prev: ImageEffects) => {
      const newShadow = {
        ...prev.shadow,
        [property]: value,
  ...
  const handleOutlineChange = (property: keyof ImageEffects["outline"], value: any) => {
    setImageEffects((prev: ImageEffects) => ({
      ...prev,
      outline: {
        ...prev.outline,
  ...
  const handleGlowChange = (property: keyof ImageEffects["glow"], value: any) => {
    setImageEffects((prev: ImageEffects) => {
      const newGlow = {
        ...prev.glow,
        [property]: value,
  ...
  const handleBackgroundChange = (property: keyof BackgroundData, value: any) => {
  ...
  const drawGlow = useCallback((ctx: CanvasRenderingContext2D, glow: ImageEffects["glow"]) => {
    ...
    switch (glow.type) {
      ...
      case "chromatic":
        ...
        const chromaticColors = [glow.color, glow.color2 || "#FF0000", glow.color3 || "#00FF00", "#0000FF", "#FF00FF"]
      ...
      case "aurora":
        ...
        const auroraColors = [glow.color, glow.color2 || "#00FF00", glow.color3 || "#0080FF", "#8000FF", "#FF0080"]
  ...
  const drawOutline = useCallback((ctx: CanvasRenderingContext2D, outline: ImageEffects["outline"]) => {
    ...
    switch (outline.type) {
      ...
      case "corners":
        ...
        // Bottom-right corner
        ctx.strokeRect(
          outlineRect.x + outlineRect.w - cornerWidth,
          outlineRect.y + outlineRect.h - cornerHeight,
          cornerWidth,
          cornerHeight,
        )
        break
      ...
      case "corners-gradient":
        ...
        // Top-right corner
        ctx.strokeRect(
          outlineRect.x + outlineRect.w - cornerWidthGrad,
          outlineRect.y,
          cornerWidthGrad,
          cornerHeightGrad,
        )
        ...
        ctx.strokeRect(
          outlineRect.x,
          outlineRect.y + outlineRect.h - cornerHeightGrad,
          cornerWidthGrad,
          cornerHeightGrad,
        )
        // Bottom-right corner
  ...
  const drawImage = useCallback(
    (ctx: CanvasRenderingContext2D, img: HTMLImageElement) => {
      // Apply image effects
      const filters = []
      if (imageEffects.brightness !== 100) filters.push(`brightness(${imageEffects.brightness}%)`)
      if (imageEffects.contrast !== 100) filters.push(`contrast(${imageEffects.contrast}%)`)
      if (imageEffects.saturation !== 100) filters.push(`saturate(${imageEffects.saturation}%)`)
      if (imageEffects.blur > 0) filters.push(`blur(${imageEffects.blur}px)`)
      if (imageEffects.hue !== 0) filters.push(`hue-rotate(${imageEffects.hue}deg)`)
      if (imageEffects.sepia > 0) filters.push(`sepia(${imageEffects.sepia}%)`)
      if (imageEffects.grayscale > 0) filters.push(`grayscale(${imageEffects.grayscale}%)`)
      if (imageEffects.invert > 0) filters.push(`invert(${imageEffects.invert}%)`)
          ...
          if (imageEffects.shadow.enabled) {
            ...
            if (shadowCtx) {
              ...
              if (imageEffects.shadow.type === "gradient") {
                ...
                const gradient = shadowCtx.createLinearGradient(0, 0, ctx.canvas.width, ctx.canvas.height)
                if (imageEffects.shadow.color2) {
                  gradient.addColorStop(0, hexToRgb(imageEffects.shadow.color, imageEffects.shadow.opacity / 100))
                  gradient.addColorStop(1, hexToRgb(imageEffects.shadow.color2, imageEffects.shadow.opacity / 100))
                } else {
                  gradient.addColorStop(0, hexToRgb(imageEffects.shadow.color, imageEffects.shadow.opacity / 100))
                ...
                for (let i = 0; i < data.length; i += 4) {
                  if (data[i + 3] > 0) {
                    // If pixel has alpha
                    const r = Number.parseInt(imageEffects.shadow.color.slice(1, 3), 16)
                    const g = Number.parseInt(imageEffects.shadow.color.slice(3, 5), 16)
                    const b = Number.parseInt(imageEffects.shadow.color.slice(5, 7), 16)
                    data[i] = r
                    data[i + 1] = g
                    data[i + 2] = b
              ...
              if (imageEffects.shadow.blur > 0 || imageEffects.shadow.spread > 0) {
                ...
                if (blurCtx) {
                  ...
                  if (spread > 0) {
                    ...
                    blurCtx.drawImage(shadowCanvas, centerX, centerY, shadowCanvas.width, shadowCanvas.height)
                    ...
                    blurCtx.drawImage(shadowCanvas, offsetX, offsetY, shadowCanvas.width, shadowCanvas.height)
                  }
                ...
                ctx.globalCompositeOperation = "multiply"
                ctx.globalAlpha = imageEffects.shadow.opacity / 100
                ctx.drawImage(shadowCanvas, imageEffects.shadow.offsetX, imageEffects.shadow.offsetY)
                ctx.globalCompositeOperation = "source-over"
                ctx.globalAlpha = 1
              }
            }
  ...
  const renderToolComponent = () => {
    switch (activeTool) {
      ...
      case "outline":
        return <OutlineTool imageEffects={imageEffects} onOutlineChange={handleOutlineChange} />
      case "glow":
        return <GlowTool imageEffects={imageEffects} onGlowChange={handleGlowChange} onGlowUpdate={handleGlowUpdate} />
      case "background":
        return <BackgroundTool backgroundData={backgroundData} onBackgroundChange={handleBackgroundChange} />
      case "transform":
        return <TransformTool imageEffects={imageEffects} onSliderChange={handleSliderChange} />
      case "crop":
        return (
          <div className="space-y-6">
            <div className="text-center text-slate-600">
              ...
              <p className="text-sm">Use the transform tools to adjust your image size and position</p>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 overflow-x-hidden">
      <Header title="Photo Editor" subtitle="Advanced editing tools & effects" />
          ...
          <div className="space-y-4 hidden sm:block">
            ...
            <div className="bg-gradient-to-br from-purple-100 to-pink-100 rounded-2xl p-6 shadow-xl border border-purple-200/50">
              <div className="text-center">
                ...
                <h3 className="text-lg font-semibold text-slate-900 mb-2">Premium Features</h3>
                <p className="text-sm text-slate-600 mb-4">Unlock advanced editing tools and AI-powered effects</p>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-xl border border-slate-200/50 hidden sm:block">
              <h3 className="text-lg font-semibold text-slate-900 mb-4">Image Info</h3>
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-medium text-center ${
                        activeTool === tool.id ? "text-purple-700" : "text-slate-700"
                      }`}
                    >
                      {tool.name}
                    </span>
          ...
          {activeTool && (
            <div
              className={`transition-all duration-500 ease-in-out ${
                activeTool ? "transform translate-x-0 opacity-100" : "transform translate-x-full opacity-0"
              }`}
            >
              {/* Tool Header */}
              <div className="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-4 border-b border-slate-200 rounded-t-2xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    >
                      {(() => {
                        const tool = toolCategories.find((t) => t.id === activeTool)
                        if (tool) {
                          const IconComponent = tool.icon
                    ...
                    <p className="text-sm text-slate-600">
                      {toolCategories.find((t) => t.id === activeTool)?.description}
                    </p>
                  </div>
                  <button onClick={closeTool} className="p-2 hover:bg-slate-200 rounded-lg transition-colors">
      ...
      <img ref={imageRef} alt="" style={{ display: "none" }} onLoad={handleImageLoad} />
      <img ref={backgroundImageRef} alt="" style={{ display: "none" }} onLoad={handleBackgroundImageLoad} />
      ...
      <div className="mt-8 p-6 bg-slate-50 border border-slate-200 rounded-xl max-w-2xl mx-auto">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-slate-600">
            Upload a new image to test editing without going through crop/adjust page
          </p>
